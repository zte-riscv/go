...
func rewriteValueRISCV64_OpRISCV64FMOVDstore(v *Value) bool {
	...
	// match: (FMOVDstore [off] {sym} ptr (FMVDX (MOVDconst [0])) mem)
	...
	for {
		...
		v_1_0 := v_1.Args[0]
		if v_1_0.Op != OpRISCV64MOVDconst || auxIntToInt64(v_1_0.AuxInt) != 0 {
			break
		}
		mem := v_2
...
func rewriteValueRISCV64_OpRISCV64FMOVWstore(v *Value) bool {
	...
	// match: (FMOVWstore [off] {sym} ptr (FMVSX (MOVDconst [0])) mem)
	// result: (MOVWstorezero [off] {sym} ptr mem)
	for {
		off := auxIntToInt32(v.AuxInt)
		sym := auxToSym(v.Aux)
		ptr := v_0
		if v_1.Op != OpRISCV64FMVSX {
			break
		}
		v_1_0 := v_1.Args[0]
		if v_1_0.Op != OpRISCV64MOVDconst || auxIntToInt64(v_1_0.AuxInt) != 0 {
...
func rewriteBlockRISCV64(b *Block) bool {
	...
	case BlockJumpTable:
		// match: (JumpTable idx)
		// result: (JUMPTABLE {makeJumpTableSym(b)} idx (MOVaddr <typ.Uintptr> {makeJumpTableSym(b)} (SB)))
		for {
			idx := b.Controls[0]
			v0 := b.NewValue0(b.Pos, OpRISCV64MOVaddr, typ.Uintptr)
			v0.Aux = symToAux(makeJumpTableSym(b))
			v1 := b.NewValue0(b.Pos, OpSB, typ.Uintptr)
			v0.AddArg(v1)
			b.resetWithControl2(BlockRISCV64JUMPTABLE, idx, v0)
